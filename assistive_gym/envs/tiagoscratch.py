...
class ScratchItchNewEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super(ScratchItchNewEnv, self).__init__(robot=robot, human=human, task='scratch_itch', obs_robot_len=(23 + len(robot.controllable_joint_indices) - (len(robot.wheel_joint_indices) if robot.mobile else 0)), obs_human_len=(24 + len(human.controllable_joint_indices)))

    def step(self, action):
        if self.human.controllable:
            action = np.concatenate([action['robot'], action['human']])
        self.take_step(actions=action, action_multiplier=0.034)
    ...
    def reset(self):
        super(ScratchItchNewEnv, self).reset()
        ...
        p.resetBasePositionAndOrientation(self.robot.body, [-1.26, -0.4, 0], self.get_quaternion([0, 0, 0.05]))
        available_joints = [i for i in range(len(self.robot.all_joint_indices)) if
                            p.getJointInfo(self.robot.body, i)[2] != p.JOINT_FIXED]
        IK = [0.32, 0, 0, 1.1, 0.6, 0.6, 0.7, -0.4, -0.1, 2, 0.025, 0.026, 0, 0, 0, 0, 0, 0, 0, 0, 0]
