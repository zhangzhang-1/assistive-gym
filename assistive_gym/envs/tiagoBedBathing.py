...
class BedBathingNewEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super(BedBathingNewEnv, self).__init__(robot=robot, human=human, task='bed_bathing', obs_robot_len=(17 + len(robot.controllable_joint_indices) - (len(robot.wheel_joint_indices) if robot.mobile else 0)), obs_human_len=(18 + len(human.controllable_joint_indices)))
    ...
    def reset(self):
        super(BedBathingNewEnv, self).reset()
        ...
        p.resetBasePositionAndOrientation(self.robot.body, [-1.2, -0.3, 0], self.get_quaternion([0, 0, 0.05]))
        available_joints = [i for i in range(len(self.robot.all_joint_indices)) if
                            p.getJointInfo(self.robot.body, i)[2] != p.JOINT_FIXED]
        IK = [0.32, 0, 0, 1.1, 0.6, 0.4, 0.9, -0.4, -0.1, 2, 0.024, 0.024, 0, 0, 0, 0, 0, 0, 0, 0, 0]
