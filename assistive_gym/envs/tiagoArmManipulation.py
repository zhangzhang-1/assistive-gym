...
class ArmManipulationNewEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super(ArmManipulationNewEnv, self).__init__(robot=robot, human=human, task='arm_manipulation', obs_robot_len=(31 + len(robot.controllable_joint_indices) - (len(robot.wheel_joint_indices) if robot.mobile else 0)), obs_human_len=(32 + len(human.controllable_joint_indices)))
    ...
    def reset(self):
        super(ArmManipulationNewEnv, self).reset()
        ...
        # Initialize the tool in the robot's grippers
        self.tool_right.init(self.robot, self.task, self.directory, self.id, self.np_random, right=False,
                             mesh_scale=[0.001] * 3)
        self.tool_left.init(self.robot, self.task, self.directory, self.id, self.np_random, right=True,
                             mesh_scale=[0.001] * 3)
        ...
        p.resetBasePositionAndOrientation(self.robot.body, [-1.1, 1.3, 0], self.get_quaternion([0, 0, -1.3]))
        available_joints = [i for i in range(len(self.robot.all_joint_indices)) if
                            p.getJointInfo(self.robot.body, i)[2] != p.JOINT_FIXED]
        IK = [0.14, 0, 0, 1.35, 0.6, 0.6, 0.7, -0.4, -0.1, 2, 0.025, 0.026, 1.35, 0.6, 0.6, 0.7, -0.4, -0.1, 2, 0.025, 0.026]
        for ii in range(len(IK)):
            p.resetJointState(self.robot.body, available_joints[ii], IK[ii])
        self.tool_left.reset_pos_orient()
        self.tool_right.reset_pos_orient()
