...
class Demo2Env(AssistiveEnv):
    def __init__(self, robot, human):
        super(Demo2Env, self).__init__(robot=robot, human=human, task='demo',
    ...
    def step(self, action):
        ...
        reward_water, water_mouth_velocities, water_hit_human_reward = self.get_demo_rewards()
        ...
        cup_pos, cup_orient = p.multiplyTransforms(cup_pos, cup_orient, [0, 0.06, 0],
                                                   self.get_quaternion([np.pi/2.0, 0, 0]), physicsClientId=self.id)
        cup_top_center_pos, _ = p.multiplyTransforms(cup_pos, cup_orient, self.cup_top_center_offset,
                                                     [0, 0, 0, 1], physicsClientId=self.id)
        reward_distance_mouth_target = -np.linalg.norm(self.target_pos - np.array(cup_top_center_pos)) # Penalize distances between top of cup and face
    ...
    def touch_human_rewards(self):
        # Check if touched human using a cup
        # Delete such particles and give the robot a reward or penalty depending on particle status
        cup_pos, cup_orient = self.tool.get_base_pos_orient()
        cup_pos, cup_orient = p.multiplyTransforms(cup_pos, cup_orient, [0, 0.06, 0],
                                                   self.get_quaternion([np.pi/2.0, 0, 0]), physicsClientId=self.id)
        top_center_pos = np.array(p.multiplyTransforms(cup_pos, cup_orient, self.cup_top_center_offset,
                                                       [0, 0, 0, 1], physicsClientId=self.id)[0])
        bottom_center_pos = np.array(p.multiplyTransforms(cup_pos, cup_orient, self.cup_bottom_center_offset,
                                                          [0, 0, 0, 1], physicsClientId=self.id)[0])
        touch_reward = 0
        cup_hit_human_reward = 0
        cup_face_velocities = []
        distance_to_face = np.linalg.norm(self.target_pos - cup_pos)
        if distance_to_face < 0.03: # hard
        # if distance_to_mouth < 0.05: # easy
        # Delete cup and give robot a reward
            cup_reward += 10
            self.task_success += 1

        return demo_reward, cup_face_velocities, cup_hit_human_reward
    ...
    def _get_obs(self, agent=None):
        ...
        robot_obs = np.concatenate([cup_pos_real, cup_orient_real, cup_pos_real - target_pos_real,
                                    robot_joint_angles, head_pos_real, head_orient_real,
                                    [self.cup_force_on_human]]).ravel()
        if agent == 'robot':
            return robot_obs
        if self.human.controllable:
            ...
            human_obs = np.concatenate([cup_pos_human, cup_orient_human,
                                        cup_pos_human - target_pos_human, human_joint_angles,
                                        head_pos_human, head_orient_human,
                                        [self.robot_force_on_human, self.cup_force_on_human]]).ravel()
            if agent == 'human':
                return human_obs
            # Co-optimization with both human and robot controllable
    ...
    def reset(self):
        ...
        p.resetDebugVisualizerCamera(cameraDistance=1.10, cameraYaw=55,
                                     cameraPitch=-45, cameraTargetPosition=[-0.2, 0, 0.75], physicsClientId=self.id)

        # Initialize the tool in the robot's gripper
        self.tool.init(self.robot, self.task, self.directory, self.id, self.np_random,
                       right=True, mesh_scale=[0.045]*3, alpha=0.75)
        ...
        self.robot.set_gripper_open_position(self.robot.right_gripper_indices,
                                             self.robot.gripper_pos[self.task], set_instantly=True)
    ...
    def generate_target(self):
        # Set target on mouth
        self.face_pos = [0, -0.11, 0.03] if self.human.gender == 'male' else [0, -0.1, 0.03]
        head_pos, head_orient = self.human.get_pos_orient(self.human.head)
        target_pos, target_orient = p.multiplyTransforms(head_pos, head_orient,
                                                         self.face_pos, [0, 0, 0, 1], physicsClientId=self.id)
        self.target = self.create_sphere(radius=0.01, mass=0.0, pos=target_pos, collision=False, rgba=[0, 1, 0, 1])
        self.update_targets()

    def update_targets(self):
        head_pos, head_orient = self.human.get_pos_orient(self.human.head)
        target_pos, target_orient = p.multiplyTransforms(head_pos, head_orient, self.face_pos, [0, 0, 0, 1], physicsClientId=self.id)
